...
import { userLevelJSON } from '../../json/userLevelJSON';
...
export class UserEditComponent implements OnInit {
  ...
  userLevelArr = userLevelJSON;
  currentUserData: any = {
    picture: 'default.png', logoPicture: 'default.png', businessPicture: 'default.png',
    pictureChanged: false, linkdin: '', facebook: ''};
  ...
  onLogoPictureSelected(event) {
    this.currentUserData.logoPicture = event.target.files[0].name;
    this.fileType = event.target.files[0].type === '';
    if (this.fileType) {
      return;
    }
    const inputEl: HTMLInputElement = this.el.nativeElement.querySelector('#logoPicture');
    const fileCount: number = inputEl.files.length;
    const formData = new FormData();
    if (fileCount > 0) { // a file was selected
      for (let i = 0; i < fileCount; i++) {
      ...
      this.http
        .post(this._uploadPictureToMongoUrl, formData).subscribe(
          res => {
            // console.log(res);
            this.currentUserData.logoPicture = res['fileName'];
            this.currentUserData.pictureChanged = true;
          },
          err => console.log(err));
    }
  ...
  onBusinessPictureSelected(event) {
    this.currentUserData.businessPicture = event.target.files[0].name;
    this.fileType = event.target.files[0].type === '';
    if (this.fileType) {
      return;
    }
    const inputEl: HTMLInputElement = this.el.nativeElement.querySelector('#businessPicture');
    const fileCount: number = inputEl.files.length;
    const formData = new FormData();
    if (fileCount > 0) { // a file was selected
      for (let i = 0; i < fileCount; i++) {
      ...
      this.http
        .post(this._uploadPictureToMongoUrl, formData).subscribe(
          res => {
            // console.log(res);
            this.currentUserData.businessPicture = res['fileName'];
            this.currentUserData.pictureChanged = true;
          },
          err => console.log(err));
    }
