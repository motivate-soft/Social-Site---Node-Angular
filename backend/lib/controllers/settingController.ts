...
import { AvatarSchema, FirstNameSchema, LastNameSchema, CitySchema, CountrySchema, CategorySchema, CompanyNameSchema,
		 JobTitleSchema, PhoneNumberSchema, PostTitleSchema, FacebookSchema, LinkdinSchema, EmailSchema, CommentsSchema } from '../models/userDummyModel';
...
const PostTitle = mongoose.model('PostTitle', PostTitleSchema);
...
export class SettingController {
	...
	public displayPictureFromFS(req: Request, res: Response) {
		Picture.findOne({name:req.params.filename}, function (err, image) {
			if (err) {
				...
				if(image == undefined)  return res.json('Error undefined');
	...
	public loadFromLocalToDb = (req: Request, res: Response) => {
		...
		fs.readdirSync(jsonDir).forEach(function (file) {
			...
			if (file === "category.json") {
				...
				new Category({ category: data }).save().catch((err) => {
					console.log(err.message);
				});
			}
			if(file === "email.json"){
			...
			if(file === "postTitle.json"){
				PostTitle.deleteMany({}, function(err){});
				new PostTitle({postTitle: data}).save().catch((err)=>{
					console.log(err.message);
				});
			}
	...
	public loadFromLocalBusinessToDb = (req: Request, res: Response) => {
		this.loadAvatarFromLocalToDb(req, res);
		this.loadPictureFromLocalToDb(req, res);
		const jsonDir = Conf.jsonDummyURL;
		fs.readdirSync(jsonDir).forEach(function (file) {
	...
	public async createDummyData(req: Request, res: Response) {
		...
		let about = [], city = [], comments = [], companyName = [], country = [],category = [], details = [], email = [],
		...
		Category.find({}, (err, category_arr) => {
			if(err){ return res.send({'error' : 'not found category'});}
			else{
				if(category_arr[0] == undefined || category_arr[0].category == undefined){
					not_found_flag = true;
				}else{
					category = JSON.parse(category_arr[0].category);
				}
			}
			if(category.length == 0) not_found_flag = true;
		});
		...
		for(let i = 0; i < userCount; i++){
			let user = {firstName: '', lastName: '', email: '', city: '', country: '', category: '', companyName: '', jobTitle: '',
			...
			user.category=category[i % category.length].category;
	...
	public async createBusinessDummyData(req: Request, res: Response) {
		const businessCount = req.params.businessCount;

		let post_schema = new Post();
		let business_schema = new Business();
		

		let about = [], city = [], comments = [], companyName = [], country = [],category=[], details = [], email = [],
					facebook = [], firstName = [], jobTitle = [], lastName = [], linkdin = [], phoneNumber = [], postTitle = [], video = [],
					avatar = [], item_picture = [];
		let interest = Conf.interestArr;
		let type = Conf.typeArr;		
		let activity = Conf.activityArr;	
		...
		PostTitle.find({}, (err, posttitle_arr) => {
			if(err){ return res.send({'error' : 'not found postTitle'});}
			else{
				if(posttitle_arr[0] == undefined || posttitle_arr[0].postTitle == undefined){
					not_found_flag = true;
				}else{
					postTitle = JSON.parse(posttitle_arr[0].postTitle);
				}
			}
			if(postTitle.length == 0) not_found_flag = true;
		});
			...
			const result = await user_schema.save((err, user) => {
				...
				function createBusiness(){
					return new Promise(function(resolve, reject) {
						let business = {
							userName: '', email: '', city: '', country: '', businessName: '',category: '', user_id: inserted_user_id,
							phone: '', comments: '', extraBlob: '2', picture: 'default.png', logo: 'default.png',
						};
						business.userName = firstName[i % firstName.length].firstName + ' ' + lastName[i % lastName.length].lastName;
						business.email = email[i % email.length].email;
						business.city = city[i % city.length].city;
						business.country = country[i % country.length].country;
						business.category = category[i % category.length].category;
						business.businessName = companyName[i % companyName.length].companyName;
						business.phone = phoneNumber[i % phoneNumber.length].phoneNumber;
						business.comments = comments[i % comments.length].comments;
						business.picture = avatar[i % avatar.length].name;
						business.logo = avatar[i % avatar.length].name;
						business.extraBlob = '2';
			
						let business_schema = new Business(business);
						business_schema.save(function(err, item){
							resolve(item);
						});
					})
				}

				createBusiness().then(function(item){
					let post = {
						postTitle: '', pictureContentType: 'image/png', postDescription: '',  id: inserted_user_id, extraBlob: '2', picture: 'default.png', video: ''
					};
					post.postTitle = postTitle[i % postTitle.length].postTitle;
					post.postDescription = comments[i % comments.length].comments;
					post.picture = avatar[(i+1) % avatar.length].name;
					post.video = video[i % video.length].video;
					let post_schema = new Post(post);
					post_schema.save(function(err, item){
						if(i==businessCount-1){
							return res.status(200).send({ 'msg': 'create business dummy data successfully' });
						}
					});	
				});	
			});
