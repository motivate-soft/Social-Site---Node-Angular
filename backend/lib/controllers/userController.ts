...
const marketRFSImageStore = multer.diskStorage({
	destination: function (req, file, cb) {
...
const marketRFSImageUpload = multer({ storage: marketRFSImageStore }).single('file');
...
export class UserController {
					...
					if(user.active === 1){
						...
						bcrypt.compare(userData.password, user.password, function(err, match) {
							...
							if(match){
								...
								let logoPicture = user.logoPicture;
								let extraBlob = user.extraBlob;
								res.cookie('XSRF-TOKEN', userId);
								res.status(200).send({ token, role, userName, userId, picture, logoPicture, extraBlob, userCountry, userCategory});								
							}else{
	...
	public validCaptchaUrl(req: Request, res: Response){
		...
		request(verificationUrl, (error, response, body) => {
			if(body == undefined) return res.json({'responseCode': 1, 'responseDesc': 'Failed captcha verification'});
	...
	public updateUser(req: Request, res: Response) {
		...
		User.findById({_id:req.params.id}, (err,user)=>{
			...
			user.logoPicture = req.body.logoPicture;
			user.businessPicture = req.body.businessPicture;
			...
			userSave().then(function(savedUserData) {
				function isUpdate() {
						...
						Business.findOne({user_id: req.params.id},(err, business)=>{
							if(err)
								...
								if(business == null) {
									...
									business.user_id = req.params.id;
									business.category = req.body.category;
									business.businessName = req.body.companyName;
									business.logo = req.body.logoPicture;
									business.picture = req.body.businessPicture;
									if(req.body.role == 1)
										business.offerType = 'gold';
									else
										business.offerType = 'simple';
									business.extraBlob = '1';
									business.updateDate = formatUpdateDate;
									business.save()
											.then((item) => {
	...
	public displayPicture(req: Request, res: Response){
		...
		try{
			...
			gfs.files.find({ filename: req.params.filename }).toArray((err, files) => {
				...
				// Check if image
				if (files[files.length - 1].contentType === 'image/jpeg' || files[files.length - 1].contentType === 'image/png' || files[files.length - 1].contentType === 'image/gif' || files[files.length - 1].contentType === 'video/mp4') {
